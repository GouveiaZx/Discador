...
@missing_routes.get("/audio/contextos-mock")
async def listar_contextos_mock_endpoint():
    """Lista contextos de áudio - endpoint alternativo sem dependências"""
    try:
        logger.info("Listando contextos de áudio (endpoint mock)")
        contextos = [
            {
                "id": 1,
