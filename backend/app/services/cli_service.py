"""
Serviço para gerenciar CLIs (Caller Line Identification) e geração aleatória.
"""
...
class CliService:
    """Serviço para operações de CLI e geração aleatória."""
    ...
    def generar_cli_aleatorio(
        ...
        """
        Gera um CLI aleatório da lista de CLIs ativos.
        
        Args:
            excluir_cli: CLI a excluir da seleção
        ...
        # Excluir CLI específico se fornecido
        if excluir_cli:
        ...
        if not clis_disponibles:
            raise HTTPException(
                status_code=404,
                detail="Nenhum CLI disponível para seleção"
            )
        
        # Selecionar CLI aleatório
    ...
    def agregar_cli(self, cli_data: CliCreate) -> Cli:
        """
        Agrega um novo CLI à base de dados.
        ...
        if existente:
            if existente.activo:
                raise HTTPException(
                    status_code=400,
                    detail=f"CLI {validacao.numero_normalizado} já existe"
                )
            else:
    ...
    def agregar_clis_bulk(self, numeros: List[str], descripcion: str = None) -> Dict[str, Any]:
        """
        Agrega múltiples CLIs.
        
        Args:
            numeros: Lista de números CLI a agregar
            descripcion: Descripción común para todos os CLIs
            
        Returns:
            Diccionario com estadísticas do processamento
        """
        clis_agregados = 0
        clis_duplicados = 0
        clis_invalidos = 0
    ...
    def atualizar_cli(self, cli_id: int, dados_atualizacao: CliUpdate) -> Cli:
        ...
        if not cli:
            raise HTTPException(
                status_code=404,
                detail=f"CLI com ID {cli_id} não encontrado"
            )
    ...
    def obter_estatisticas(self) -> CliStatsResponse:
        """
        Obtém estatísticas dos CLIs.
        
        Returns:
            CliStatsResponse com estatísticas
        """
