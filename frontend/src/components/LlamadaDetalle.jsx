...
/**
 * Modal para exibir detalhes de uma chamada
 * 
 * @param {Object} props - Propriedades do componente
 * @returns {JSX.Element|null} Componente JSX ou null se não estiver visível
 */
const LlamadaDetalle = ({ llamadaId, isOpen, onClose }) => {
  ...
  // Carregar detalhes da chamada quando o modal for aberto
  useEffect(() => {
    if (isOpen && llamadaId) {
      setLoading(true);
      obtenerDetalleLlamada(llamadaId)
        .then(data => {
          setLlamada(data);
          setError(null);
        })
        .catch(err => {
          console.error(`Erro ao obter detalhes da chamada ID ${llamadaId}:`, err);
          setError('Erro ao obter detalhes da chamada. Tente novamente.');
        })
        .finally(() => {
          setLoading(false);
        });
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm text-gray-400">ID da Chamada</p>
